...
pub(crate) fn extract(file_path: &str, content: &str) -> Result<Vec<SymbolEntry>, String> {
    ...
    let language = match ext {
        "tsx" => tree_sitter::Language::from(tree_sitter_typescript::LANGUAGE_TSX),
        _ => tree_sitter::Language::from(tree_sitter_typescript::LANGUAGE_TYPESCRIPT),
    };
    parser
        .set_language(&language)
        .map_err(|e| format!("Failed to configure typescript parser: {}", e))?;
    let tree = parser
        .parse(content, None)
        .ok_or("Failed to parse TypeScript source".to_string())?;
...
fn visit_node(
    ...
    match node.kind() {
        "program" | "statement_block" | "class_body" | "object_type" => {
        ...
        "function_declaration" | "generator_function_declaration" => {
            push_named(node, source, "function", indent, out)
        }
        "class_declaration" => {
        ...
        "type_alias_declaration" => push_named(node, source, "type", indent, out),
        "enum_declaration" => push_named(node, source, "enum", indent, out),
        "method_definition" | "method_signature" | "abstract_method_signature" => {
            push_named(node, source, "method", indent, out)
        }
