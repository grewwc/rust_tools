use super::state::StreamFramingState;
...
pub(super) fn consume_sse_line(state: &mut StreamFramingState, line: &str) -> Option<String> {
...
pub(super) fn flush_sse_event(state: &mut StreamFramingState) -> Option<String> {
    if state.sse_event_data.trim().is_empty() {
        state.sse_event_data.clear();
        return None;
    }
    Some(std::mem::take(&mut state.sse_event_data))
}
...
mod tests {
    ...
    use crate::ai::stream::state::StreamFramingState;
