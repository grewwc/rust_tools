        ...
        match state {
            TableState::None => {
                if !self.in_code_block && is_table_row_candidate(line) && !is_table_separator(line)
                {
                    ...
                    out.push_str(&raw);
                    out.push('\n');
                    return format!("-> [State: None -> PendingHeader] out: {:?}", out);
                }
                return format!("-> [State: None -> None] rendered");
            }
            TableState::PendingHeader {
                ...
                if is_table_separator(line) {
                    ...
                    return format!("-> [State: PendingHeader -> InTable] out: {:?}", out);
                }

                self.table_state = TableState::None;
                return format!("-> [State: PendingHeader -> None] Restored header and consumed line");
            }
            TableState::InTable {
                indent,
                header,
                ...
                if is_table_row_candidate(line) {
                    ...
                    return format!("-> [State: InTable -> InTable] out: {:?}", out);
                }
                self.table_state = TableState::None;
                return format!("-> [State: InTable -> None] Broken out of table");
            }
        }
    }
}
...
fn main() {
    let mut r = MarkdownStreamRenderer {
        table_state: TableState::None,
        in_code_block: false,
        bol: true,
        line_preview_height: 1,
    };
    
    let l1 = " | 知识类型 | 示例 | 变化频率 | 验证方式 | ";
    let l2 = " |---------|------|---------|---------| ";
    let l3 = " | **文件/代码** | 项目结构、函数实现 | 低（手动修改） | 文件指纹 | ";
    
    println!("L1: {}", r.consume_line(l1, false));
    println!("L2: {}", r.consume_line(l2, false));
    println!("L3: {}", r.consume_line(l3, false));
}
