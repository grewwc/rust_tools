...
impl FilterChain {
    ...
    pub(crate) fn push<F: StreamFilter + 'static>(mut self, f: F) -> Self {
        self.filters.push(Arc::new(f));
        self
    }
    pub(crate) fn push_boxed(mut self, f: Box<dyn StreamFilter>) -> Self {
        self.filters.push(Arc::from(f));
        self
    }
    pub(crate) fn register<F: StreamFilter + 'static>(&mut self, filter: F) {
...
impl<D: StreamDecoder> FilteredStreamDecoder<D> {
    ...
    pub(crate) fn with_before(mut self, chain: FilterChain) -> Self {
        self.before = chain;
        self
    }
    pub(crate) fn with_after(mut self, chain: FilterChain) -> Self {
        self.after = chain;
        self
    }
    pub(crate) fn before_chain_mut(&mut self) -> &mut FilterChain {
        &mut self.before
    }
    pub(crate) fn after_chain_mut(&mut self) -> &mut FilterChain {
        &mut self.after
    }
}
impl<D: StreamDecoder> StreamDecoder for FilteredStreamDecoder<D> {
    fn decode<'a>(
        ...
        Box<
            dyn Future<Output = Result<DecodedStream, Box<dyn std::error::Error + Send + Sync>>>
                + Send
                + 'a,
        >,
    > {
        ...
        self.inner
            .decode(app, response, current_history, terminal_dedupe_candidate)
    }
}
