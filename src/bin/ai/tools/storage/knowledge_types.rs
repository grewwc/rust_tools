...
impl KnowledgeType {
    ...
    pub fn infer_from_description(desc: &str) -> Self {
        ...
        // 时间敏感关键词
        if desc_lower.contains("天气")
            || desc_lower.contains("weather")
            || desc_lower.contains("今天")
            || desc_lower.contains("today")
            || desc_lower.contains("明天")
            || desc_lower.contains("tomorrow")
            || desc_lower.contains("现在")
            || desc_lower.contains("now")
            || desc_lower.contains("当前")
            || desc_lower.contains("current")
        {
        ...
        if desc_lower.contains("api")
            || desc_lower.contains("状态")
            || desc_lower.contains("status")
            || desc_lower.contains("价格")
            || desc_lower.contains("price")
            || desc_lower.contains("股票")
        ...
        // 文件/代码关键词
        if desc_lower.contains("项目结构")
            || desc_lower.contains("project structure")
            || desc_lower.contains("代码")
            || desc_lower.contains("code")
            || desc_lower.contains("文件")
        ...
        // 稳定知识关键词
        if desc_lower.contains("规范")
            || desc_lower.contains("guideline")
            || desc_lower.contains("最佳实践")
            || desc_lower.contains("best practice")
            || desc_lower.contains("原则")
            || desc_lower.contains("principle")
        {
...
pub enum ValidationStrategy {
    ...
    /// 时间范围验证
    TimeRange { valid_from: u64, valid_until: u64 },
    ...
    pub fn new(
        ...
        let validation = match &knowledge_type {
            ...
            KnowledgeType::SessionScoped => ValidationStrategy::SessionBound {
                session_id: context
                    .get_ref(&"session_id".to_string())
                    .cloned()
                    .unwrap_or_else(|| format!("session_{}", now)),
            },
