...
4. **Subagent spawn depth.** `runtime_ctx::SUBAGENT_DEPTH` (task-local) tracks nesting; `MAX_SUBAGENT_SPAWN_DEPTH` (=1) allows only top-level agent → child subagent. Child subagents do not receive task orchestration tools (`task`/`task_spawn`/`task_wait`/`task_status`) in their visible tool set, and direct calls still fail by depth guard. Depth carried in `OsTaskGoal.spawn_depth`, scoped by both dispatch paths.
