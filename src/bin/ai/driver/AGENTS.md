...
12. **Final-response gates (completion, citation).** Conservative gates sharing one reopen per turn (`FinalGateState`).
    - **Completion.** Purely structural — the final's wording is never classified, because positive and negative word lists are both inherently incomplete. After a provable `apply_patch`/`write_file` mutation, only the tool record decides: command-level "mutation" is intent classification (silent Allow), temp-only side effects don't count, any successful post-mutation activity (including unrecognized `python3` scripts or read-only tools) silently Allows, so only the provable zero-activity state reopens once then warns, whatever the final says. Honest "cannot verify" replies and promised follow-up checks are not exempt: the reopen note tells the model to inspect the diff, run the narrowest check, or state the limitation explicitly (non-mutation turns keep the dangling gate for action-promise finals). A confirmed failed `cargo check` after mutation overrides and warns immediately; warning-only finals still get one no-tool synthesis grace.
