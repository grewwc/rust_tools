...
mod tests {
    ...
    fn turn_hooks_fire_on_app_start_and_end() {
        ...
        app.hooks
            .register_before(StageKind::Prepare, "start", move |ctx| {
        ...
        app.hooks
            .register_after(StageKind::Finalize, "end", move |ctx| {
    ...
    fn turn_hook_failure_is_logged_not_fatal() {
        let mut app = crate::ai::middleware::test_util::test_app();
        app.hooks
            .register_before(StageKind::Prepare, "fail", |_ctx| Err("boom".into()));
    ...
    fn intermediate_hooks_fire_only_stage_hooks() {
        ...
        let f = std::sync::Arc::clone(&fired);
        app.hooks
            .register_before(StageKind::BuildRequest, "before_request", move |_ctx| {
                f.lock().unwrap().push("before_request".to_string());
                Ok(())
            });
        let f = std::sync::Arc::clone(&fired);
        app.hooks
            .register_after(StageKind::ParseStream, "after_stream", move |_ctx| {
                f.lock().unwrap().push("after_stream".to_string());
                Ok(())
            });
        let f = std::sync::Arc::clone(&fired);
        app.hooks
            .register_before(StageKind::ExecuteTools, "before_tools", move |_ctx| {
        ...
        assert_eq!(
            *fired.lock().unwrap(),
            vec![
                ...
                "after_stream".to_string(),
                "before_tools".to_string()
            ]
        );
    }
    ...
    fn intermediate_hook_failure_is_logged_not_fatal() {
        let mut app = crate::ai::middleware::test_util::test_app();
        app.hooks
            .register_before(StageKind::BuildRequest, "fail", |_ctx| Err("boom".into()));
        app.fire_before_request_hooks(&mut Vec::new());
        app.fire_after_stream_hooks();
        app.fire_before_tools_hooks();
        // 失败只记录，注册表不丢失
    ...
    fn before_request_hook_sees_and_modifies_real_messages() {
        let mut app = crate::ai::middleware::test_util::test_app();
        app.hooks
            .register_before(StageKind::BuildRequest, "req", |ctx| {
    ...
    fn before_request_hook_failure_still_restores_messages() {
        let mut app = crate::ai::middleware::test_util::test_app();
        app.hooks
            .register_before(StageKind::BuildRequest, "fail", |ctx| {
                ctx.messages.push(Message {
                    role: "assistant".to_string(),
