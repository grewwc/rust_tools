...
//! 文本截断，以及每操作超时包装。
//!
//! 骨架照抄 `src/bin/mcp_ocr.rs`，但改造成 async（tokio stdout，每行写完 flush）。
...
pub fn cap_text(s: &str) -> String {
    ...
    format!(
        "{truncated}\n\n[... truncated: output exceeded {CAP_CHARS} chars; refine your selector or use evaluate_js to extract a smaller slice ...]"
    )
}
...
/// 给每个 CDP 操作套一个 server 内部上限（默认 90s，短于宿主 120s 的
...
/// 否则宿主会 kill+restart 子进程、丢掉整个浏览器会话。
    ...
    F: std::future::Future<Output = Result<T, String>>,
{
    match tokio::time::timeout(std::time::Duration::from_millis(ms), fut).await {
        ...
        Err(_) => Err(JsonRpcErr::new(
            -32001,
            &format!("operation reached the {ms} ms mcp_browser server cap"),
            None,
        )),
    }
}
