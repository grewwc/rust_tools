...
3. **Progressive loading.** Default `core` (`DEFAULT_TURN_TOOL_GROUPS` in `driver/skill_runtime.rs`). Lazy tools load via `enable_tools` — by tool name or by group shortcut (`knowledge`, `skills`, `task`, `executor`, ...; `builtin` itself is only the enable-ability flag and never expands). `os_tools` need `executor` plus `builtin`. Explicit `tools:` lists pin eager visibility.
