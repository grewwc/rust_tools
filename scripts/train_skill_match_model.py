...
def build_dataset(corpus: dict):
    ...
    for sample in samples:
        label = sample.get("skill") or sample.get("agent") or sample.get("core")
    ...
    for doc in docs:
        ...
        xs.append(vec)
        ys.append(label_to_idx.get(doc["label"], -1))
...
def evaluate(labels, vocab, weights, bias, xs, ys):
    ...
    for vec, target in zip(xs, ys):
        if predict_one(labels, vocab, weights, bias, vec)[0] == target:
            correct += 1
...
def main():
    parser = argparse.ArgumentParser(description="Train local TF-IDF + Logistic Regression skill match model")
    parser.add_argument(
        "--corpus",
        default="src/bin/ai/config/skill_match/training_corpus.json",
        help="Path to training corpus json",
    )
    parser.add_argument(
        "--output",
        default="src/bin/ai/config/skill_match/skill_match_model.json",
