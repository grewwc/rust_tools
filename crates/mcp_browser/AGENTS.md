...
5. **Synthetic events only.** JS `click()`, native-setter `value`+`input`/`change`, `KeyboardEvent` dispatch (Enter also `form.requestSubmit()`). No OS key chords/drag&drop. `full_page` 降级为窗口截图；`screenshot` 先跑无提示的 TCC 预检（`CGPreflightScreenCaptureAccess`，`screen_capture_allowed()`）：缺 *Screen Recording* 权限时 `screencapture -l` 必败而区域/全屏回退仍 exit 0 但产出仅含壁纸的假成功图（实测：预检 false 时 4 个真实 Chrome 窗口全部 "could not create image from window"），故立即报错并给出授权指引（macOS 15+ 每月重验一次该授权）；有权限时三段回退 `screencapture -l <wid>` → `bounds → -R x,y,w,h` → 全屏，首次调用无窗口时自动 `about:blank` 建会话。授权后需完全退出并重启宿主应用才生效。
