...
8. **Tool results are capped.** Oversized output goes to session overflow file with a self-describing stub; apply same cap when rebuilding context without mutating history.
...
16. **Validated claims are runtime-derived.** For manifest-enabled agents, non-mutating
    turns with multiple successful tool results use a versioned structured final
    payload so unclassified inspection tools cannot bypass the gate. Resolve receipts only
    from stable replay-safe canonical current-turn results; accept only complete exact
    field/value evidence; derive identity with closed request/session/trace key rules; render
    verified conclusions in the driver. Unsupported joins remain explicitly unverified and
    truncation cannot bypass the gate.
