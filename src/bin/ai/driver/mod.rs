...
fn try_handle_memory_command(
    ...
    if parts.len() < 2 {
        ...
        println!("  /memory save <content> [--category <cat>] [--tags <t1,t2>] [--source <src>]");
        println!("  /memory recent [limit]");
        println!("  /memory search <query> [--limit <n>] [--category <cat>]");
        println!();
        println!("Examples:");
        println!("  /memory save Rust 项目优先使用 cargo check 进行快速验证 --category coding_guideline --tags rust,cargo");
        println!("  /memory recent 5");
        println!("  /memory search cargo --limit 10 --category coding_guideline");
        return Ok(true);
    }
    ...
    match parts[1] {
        "save" => {
            // 解析 save 命令的参数
            let content_start = normalized.find("save").map(|i| i + 4).unwrap_or(0);
            let content_and_flags = normalized[content_start..].trim();
            
            // 分离内容和标志
            let mut content_parts = Vec::new();
            let mut category = "user_memory".to_string();
            let mut tags: Vec<String> = vec!["user_directed".to_string()];
            let mut source = Some("user_command".to_string());
            
            let mut flag_parts = content_and_flags.split("--");
            let content_str = flag_parts.next().unwrap_or("").trim();
            content_parts.push(content_str);
            
            for part in flag_parts {
                let part = part.trim();
                if part.starts_with("category") {
                    category = part["category".len()..].trim().trim_matches('"').trim_matches('\'').to_string();
                } else if part.starts_with("tags") {
                    let tags_str = part["tags".len()..].trim().trim_matches('"').trim_matches('\'');
                    tags = tags_str.split(',').map(|s| s.trim().to_string()).filter(|s| !s.is_empty()).collect();
                } else if part.starts_with("source") {
                    source = Some(part["source".len()..].trim().trim_matches('"').trim_matches('\'').to_string());
                }
            }
            
            let content = content_parts.join(" ").trim().to_string();
            if content.is_empty() {
                println!("Error: content is required. Usage: /memory save <content> [--category <cat>]");
                return Ok(true);
            }
            ...
            match crate::ai::tools::service::memory::execute_memory_save(&args) {
                Ok(msg) => println!("{}", msg),
                Err(e) => eprintln!("Error: {}", e),
            }
        }
        "recent" => {
            let limit = parts.get(2).and_then(|s| s.parse::<usize>().ok()).unwrap_or(10).min(50);
            let args = serde_json::json!({ "limit": limit });
            match crate::ai::tools::service::memory::execute_memory_recent(&args) {
                Ok(msg) => println!("{}", msg),
                Err(e) => eprintln!("Error: {}", e),
            }
        }
        "search" => {
            // 简单解析：/memory search <query> [--limit <n>] [--category <cat>]
            let query_start = normalized.find("search").map(|i| i + 6).unwrap_or(0);
            let search_args = normalized[query_start..].trim();
            
            let query;
            let mut limit = 10usize;
            let mut category: Option<String> = None;
            
            let mut arg_parts = search_args.split("--");
            query = arg_parts.next().unwrap_or("").trim().to_string();
            
            for part in arg_parts {
                let part = part.trim();
                if part.starts_with("limit") {
                    limit = part["limit".len()..].trim().parse().unwrap_or(10).min(50);
                } else if part.starts_with("category") {
                    category = Some(part["category".len()..].trim().trim_matches('"').to_string());
                }
            }
            
            if query.is_empty() {
                println!("Error: query is required. Usage: /memory search <query>");
                return Ok(true);
            }
        ...
        _ => {
            println!("Unknown /memory command. Try: /memory help");
        }
    }
