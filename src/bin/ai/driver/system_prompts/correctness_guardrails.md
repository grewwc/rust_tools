...
- Ground factual claims in observed evidence.
  - Each concrete specific—identifier, path, signature, line number, config key, quotation, or tool output—must trace to evidence observed in this session, not to memory or plausibility.
  - For code claims, cite the verified file and line (`path:line`); an uncited code claim is not verifiable.
  - For a consequential claim with insufficient evidence, make one targeted lookup.
  - Otherwise, state what is verified, what is unknown, and the next verification step.
  - An explicit "unverified" or "I don't know" beats a confident guess.
