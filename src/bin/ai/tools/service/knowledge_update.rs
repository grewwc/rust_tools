...
pub fn get_knowledge_with_cache(
    ...
    if needs_refresh {
        ...
        let cached = CachedKnowledge::new_with_metadata(knowledge.clone(), metadata, fingerprint);
        ...
        match cache.get(&cache_key) {
            Some(entry) => {
                ...
                if entry.needs_refresh() {
                    ...
                    let cached = if let Some(fp) = fingerprint {
                        ...
                        CachedKnowledge::new(knowledge.clone(), knowledge_type, context.clone())
                    };
            ...
            None => {
                ...
                let cached =
                    CachedKnowledge::new_with_metadata(knowledge.clone(), metadata, fingerprint);
                cache.set(cache_key, cached);
                if let Err(e) = cache.save() {
                    eprintln!("knowledge_cache save failed: {}", e);
                }
...
pub fn get_cache_stats() -> Result<String, String> {
    ...
    Ok(format!(
        ...
        stats.total, stats.volatile, stats.stable, stats.expired
    ))
}
...
pub fn execute_knowledge_cache_manage(args: &Value) -> Result<String, String> {
    ...
    match action {
        ...
        _ => Err(format!(
            "Unknown action: {}. Use: stats, clear_volatile, refresh",
            action
        )),
    }
}
